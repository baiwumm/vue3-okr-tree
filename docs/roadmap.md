...
| 1.16.0 | `virtual` 虚拟滚动（#15 收官，上游 2.x 清单 #10–#16 全部完成）                     | 🚧 待发布 |
...
| 2026-09-26 | 站务   | 文档站 VitePress 1.6 → **Docus v5**（Nuxt layer，官方 @nuxt/content v3 + @nuxt/ui v4）：`docs-site/` → `docs-site-docus/`，Demo 单一来源复用不变（MDC 包装组件），部署产物 `.vitepress/dist` → `.output/public`（wrangler.jsonc 同步，CF Pages 面板的输出目录设置需同步改）；详见 `docs-site-docus/README.md` | 随下次提交入库                |
