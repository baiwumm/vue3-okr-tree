...
- [x] 技术预研先行（✅ 2026-09-26 spike，探针 `_scratch/spike-virtual.spec.ts`：真实 Chromium + UMD 产物，1 父 + 10000 平铺子节点实测）：
...
- **验收（2026-09-26 实测收口，指标改写为可守的两条 + 一条诚实记录）**：~~10000 平铺子节点首帧 < 300ms~~ → **① DOM 有界**：1 父 + 10000 平铺子节点渲染节点 10001 → **13**，滚动 / 揭示 / 展开全程有界；**② 几何完整**：行总宽与全量渲染逐像素一致（±2px 断言）、窗口边界帽与占位块连线段真实衔接、键盘漫游 30 步逐条 +1 不跳焦；**③ 诚实记录**：首帧成本的大头是 **store 构建**（1 万节点约 2.9s，与 virtual 无关、全量渲染同样存在，已单列为独立性能待办），virtual 消除的是 DOM 数量与滚动 / 展开时的渲染卡顿——排除 store 后首帧渲染约 55ms。
...
| 2026-09-26 | 1.16.0 | **#15 收官（上游 2.x 清单 #10–#16 全部完成）**：`virtual` 虚拟滚动——宽度模型 + 等尺寸占位块保几何（行宽与全量渲染 ±2px）、边界帽自动正确、展开行模型定宽、scrollToNode / 键盘漫游先揭示再定位；单测 +13、visual +3（零新增 png）、预算 19/19.5 → 21 kB；store 构建 2.9s/万节点单列性能待办                                                                                                            | `c59cc6c`                     |
| 2026-09-26 | 站务   | 文档站 VitePress 1.6 → **Docus v5**（Nuxt layer，官方 @nuxt/content v3 + @nuxt/ui v4），目录沿用 `docs-site/`（迁移中途曾命名 `docs-site-docus/`，2026-09-26 入库前改回，未进过 git 历史），Demo 单一来源复用不变（MDC 包装组件），部署产物 `.vitepress/dist` → `.output/public`（wrangler.jsonc 同步，CF Pages 面板的输出目录设置需同步改为 `docs-site/.output/public`）；详见 `docs-site/README.md` | 随下次提交入库                |
