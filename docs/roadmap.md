...
| 1.14.2 | G8 Demo 交互层断言 + 四条库侧修复（对外 API 形状零变化）                           | ✅ 已发布 |
| 1.15.0 | Vue Devtools 面板（#16 收官）+ `DEFAULT_PROPS` 导出补齐（两仓导出面 19 ↔ 19 对齐） | ✅ 已发布 |
| 1.16.0 | `virtual` 虚拟滚动（#15 收官，上游 2.x 清单 #10–#16 全部完成）                     | ✅ 已发布 |
| 存量项 | ~~#12 更多布局~~ 挂起等真实需求；#10–#15 已全部完成、#16 随 1.15.0 完成            | —         |
...
- [x] `connector: 'css' | 'svg'`，默认 `css`（现状）
- [x] `svg` 模式：测量节点位置，覆盖层 `<svg>` 绘制路径；支持 `curve` / `orthogonal` / `straight`（布局零改动，仅中和连线伪元素边框）
- [x] 线宽/颜色继续走 `--okr-line-*` 变量；onUpdated + ResizeObserver + animate 过渡期 rAF 连续重绘，无残影
- **验收**：曲线模式下拖动/展开无残影，性能与 CSS 模式同量级（≤ 500 节点）。✅ 2026-09-19 完成（12 个专项测试）
...
- [x] 技术预研先行（✅ 2026-09-26 spike，探针是本地临时件、沿例不入库（见 `fbaffdc`），真实 Chromium + UMD 产物，1 父 + 10000 平铺子节点实测）：
  - **问题坐实**：基线首帧 **3.8–4.5s**（验收线 300ms 的 12 倍以上），10001 个 DOM 节点全量渲染；
  - **瓶颈在框架层不在渲染层**：`content-visibility: auto` 首帧 3.9s（噪声级改善）——1 万个组件实例化 + store 注册的成本 CSS 跳不掉，**cv 路线排除**（顺带证明 cv 下连接线与 float 布局完好：中间兄弟三线段全 1px、相邻间隙 0）；
  - **连接线的相邻依赖比预想窄**：每个节点自绘全部线段（`::before` 左半段、`::after` 右半段 + 穿自身的竖线），真正吃 DOM 相邻的只有边界帽——`:first-child` / `:last-child` / `:only-child` 的去线与圆角。实测删 DOM 窗口化后，窗口首末节点被误帽（`border-top: 0px`，而两侧各有 5000 个真实兄弟在模型里）——**可修且范围有限**：窗口边界加覆盖类恢复整线即可；
  - **spacer 行高路线排除**：占位行没有线段，横条在隐藏区断线；且 float 不撑高容器（实测容器高度仅剩 `::before` 桩的 20px），真·窗口化必须自算容器尺寸与条目 x 偏移——`labelWidth` 固定卡宽是现成的有利条件；
  - **结论：可做，是真 feature 不是快赢**。实现面 = OkrTreeNode 列表窗口化 + 边界帽覆盖类 + 容器显式尺寸；兼容面 = 展开动画、`scrollToNode`（先渲染再滚动）、键盘漫游（焦点进离屏项需先渲染）、`filter` 四项逐个验。
- [x] 与 `<OkrTreeViewport>` 互补：仅对展开后的可见 treeitem 做窗口化渲染（✅ 2026-09-26，`virtual: boolean` 组件 prop + viewport 平移 / 缩放经 MutationObserver 触发窗口重算，可同时开启）
- [x] 与展开动画、`scrollToNode`、键盘漫游 tabindex、`filter` 的兼容性逐项验证（✅ 单测 13 条 + 浏览器 2 条 + demo 交互 1 条：窗口化口径按「过滤后可见兄弟」，aria / show-node-num / `getVisibleNodes()` 按全量可见列表；`scrollToNode` 与方向键 / Home / End 对窗口外目标**先揭示再定位**，逐条推进不跳焦；展开动画照常，折叠行不渲染占位块以保持折叠宽度一致）
- [x] prop：`virtual: boolean`（✅ 组件 prop；要求数字型 `label-width`，horizontal 另要求 `label-height`，auto 退回全量渲染并警告；创建期快照）
- **验收（2026-09-26 实测收口，指标改写为可守的两条 + 一条诚实记录）**：~~10000 平铺子节点首帧 < 300ms~~ → **① DOM 有界**：1 父 + 10000 平铺子节点渲染节点 10001 → **13**，滚动 / 揭示 / 展开全程有界；**② 几何完整**：行总宽与全量渲染逐像素一致（±2px 断言）、窗口边界帽与占位块连线段真实衔接、键盘漫游 30 步逐条 +1 不跳焦；**③ 诚实记录**：首帧成本的大头是 **store 构建**（1 万节点约 2.9s，与 virtual 无关、全量渲染同样存在，~~已单列为独立性能待办~~ → 已记账在 `docs/perf.md`「万级平铺」一节，2026-09-26 起 `pnpm bench` 的万级两条场景可复现；**明确不设门禁**，动模型层才有解且当前无需求信号），virtual 消除的是 DOM 数量与滚动 / 展开时的渲染卡顿——排除 store 后首帧渲染约 55ms。
...
| 2026-09-25 | 1.14.2 | G8 收口：24 个 Demo 的交互层断言（22 条浏览器用例，视觉用例 21 → 43 条、零新增像素基线），顺带修掉 4 处 Demo 自身缺陷；三条库语义修复（拖拽 `node-drag-end` 载荷、左树过滤保根、`default-checked-keys` 按内容重放）+ 画布平移甩出边界松手卡手势修复                                                                                                                                                                                           | `2649961` `a324603` `60dceaf` |
| 2026-09-26 | 1.15.0 | **#16 收官**：Vue Devtools 面板（dev only，零依赖直挂 `__VUE_DEVTOOLS_GLOBAL_HOOK__`，实例列表 / 概要 / 左右树节点注册表转储，+1.5 kB gzip 进产物、消费端生产构建消除）+ `DEFAULT_PROPS` 导出补齐（两仓导出面最后一个不对称清零）；单测 +8、verify:dist +2                                                                                                                                                                                    | `f19a0d9`                     |
| 2026-09-26 | 1.16.0 | **#15 收官（上游 2.x 清单 #10–#16 全部完成）**：`virtual` 虚拟滚动——宽度模型 + 等尺寸占位块保几何（行宽与全量渲染 ±2px）、边界帽自动正确、展开行模型定宽、scrollToNode / 键盘漫游先揭示再定位；单测 +13、visual +3（零新增 png）、预算 19/19.5 → 21 kB；store 构建 2.9s/万节点单列性能待办（→ 2026-09-26 已记账 `docs/perf.md`「万级平铺」，不设门禁）                                                                                        | `c59cc6c`                     |
| 2026-09-26 | 站务   | 收口批（库源码零改动）：万级 store 构建账落地（`pnpm bench` 两条万级场景 + `perf.md` 新节，明确不设门禁）、**文档站进 CI**（`ci.yml` 新增 Docs site build job，跑 Cloudflare 用的同一条 `docs:build:full`）、roadmap 三处过期措辞就地更正（spike 探针入库状态、store 待办去向、CF 面板已验证）                                                                                                                                                | 本次提交                      |
| 2026-09-26 | 站务   | 文档站 VitePress 1.6 → **Docus v5**（Nuxt layer，官方 @nuxt/content v3 + @nuxt/ui v4），目录沿用 `docs-site/`（迁移中途曾命名 `docs-site-docus/`，2026-09-26 入库前改回，未进过 git 历史），Demo 单一来源复用不变（MDC 包装组件），部署产物 `.vitepress/dist` → `.output/public`（wrangler.jsonc 同步，~~CF Pages 面板的输出目录设置需同步改为 `docs-site/.output/public`~~ → 2026-09-26 面板已改并线上验证通过）；详见 `docs-site/README.md` | `b181fca`                     |
