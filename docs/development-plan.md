...
> ⚠️ **本文是 1.0.0 开工期（2026-09-16 → 09-17）的计划与核对快照，全部条目已完成，原样封存**。文中用例数、断言数、包体积等均为当时点数值，请勿当作现状引用——最新门禁数字见 `acceptance.md`，后续版本推进见 `roadmap.md`。
...
| 2   | Demo 页 24 个用例可交互、与原 Demo 对齐；Filter 体现空值恢复                   | ✅   | 浏览器逐用例截图核对：三模式渲染、+/- 展开收起、节点数、选中样式、render-content / node-btn-content / 插槽、动画过渡、OKR 根对齐（开/关、收起不位移）、filter 与空值恢复、11 个方法按钮、4 类事件日志；无控制台 error/warn |
